...
public class Librarian3 {
    ...
    public boolean orderNewBook(String studentId, Calendar calendar,
                                BookType bookType, String bookId) {
        ...
        if (add) {
            ...
            System.out.println("(Sequence) " + "[" + calendar.get(Calendar.YEAR) + "-" +
                   String.format("%02d", (calendar.get(Calendar.MONTH) + 1)) + "-" +
                  String.format("%02d", calendar.get(Calendar.DATE)) + "] " +
                   "School" + " sends a message to " + "Librarian3");
        ...
        while (it1.hasNext()) {
            ...
            while (it2.hasNext()) {
                ...
                if (book1.getType() == book2.getType() &&
                    ...
                    students.get(studentId).getOrderedBook(book2, calendar);
                    //TODO 学生成功借书时输出
                    System.out.println("[" + calendar.get(Calendar.YEAR) + "-" +
                            String.format("%02d", (calendar.get(Calendar.MONTH) + 1)) + "-" +
                            String.format("%02d", calendar.get(Calendar.DATE)) + "] " +
        ...
        for (Item item : toPurchase) {
            ...
            for (int i = 0; i < n; ++i) {
                Book book = new Book(bookType, bookId,
                        true, schoolName, State.NOT_BORROWED, null);
