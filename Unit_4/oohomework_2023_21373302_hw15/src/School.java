...
public class School {
    ...
    public void initialBooks(BookType bookType, String bookId,
        ...
        for (int i = 0; i < n; ++i) {
            shelf.addBook(new Book(bookType, bookId, ub, schoolName, State.NOT_BORROWED, null));
        }
    }
        ...
        if (machine.queryBook(bookType, bookId, calendar, studentId)) {
            ...
            if (book.getType() == BookType.A) {
                ...
                if (librarian1.borrowBook(studentId, book, calendar)) {
                    students.get(studentId).getOrderedBook(book, calendar); // 这里
        ...
        for (Message message : cannotSatisfyList) {
            if (message.isValid() && Objects.equals(message.getSchoolName(), schoolName)) {
                ...
                if (ownBook(bookType, bookId)) {
                    //如果图书馆有这本书，那么就预定它
                    librarian3.orderNewBook(studentId, calendar, bookType, bookId);
                } else {
                    //否则，需要部门去购买
                    if (librarian3.orderNewBook(studentId, calendar, bookType, bookId)) {
